...
function TitledList(props) {
    ...
    let [isOpen, setOpen] = useState (props.force ?? true);
    ...
    return (
        <div style={{...props.style}} className='app-lateral-toolbar'>
            <div className='toolbar-title' onClick={()=>setOpen (!isOpen)} style={{cursor: 'pointer'}}>
                {(props.force ?? isOpen) ? <MdExpandMore/> : <MdExpandLess/>}
            ...
            <Rnd className='list-container' style={{position: 'block', display: 'flex'}} disableDragging={true}
                ...
                default={{x: 0, y: 27, height: 200}} size={{height: (props.force ?? isOpen) ? height : 0}} minHeight={5} maxHeight={window.innerHeight / 3.7}
